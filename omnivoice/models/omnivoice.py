...
class OmniVoice(PreTrainedModel):
    ...
    def __init__(self, config: OmniVoiceConfig, llm: Optional[PreTrainedModel] = None):
        ...
        self._asr_pipe = None
        self.token_id_mapping = None

    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
        train_mode = kwargs.pop("train", False)
        load_asr = kwargs.pop("load_asr", False)
        asr_model_name = kwargs.pop("asr_model_name", "openai/whisper-base")
        ...
        try:
            ...
            if not train_mode:
                ...
                # Load vocabulary mapping if it exists (for pruned models)
                mapping_path = os.path.join(resolved_path, "vocab_mapping.json")
                if os.path.exists(mapping_path):
                    try:
                        import json
                        with open(mapping_path, "r", encoding="utf-8") as f:
                            mapping_dict = json.load(f)
                        mapping_list = [0] * 151676
                        default_mapped_id = mapping_dict.get("151643", 0) # default to mapped pad token
                        for i in range(151676):
                            mapping_list[i] = mapping_dict.get(str(i), default_mapped_id)
                        model.token_id_mapping = torch.tensor(mapping_list, dtype=torch.long)
                        logger.info("Loaded pruned vocabulary mapping from %s", mapping_path)
                    except Exception as e:
                        logger.error("Error loading vocabulary mapping: %s", e)

                if load_asr:
                    model.load_asr_model(model_name=asr_model_name, device=model.device)
        finally:
    ...
    def load_asr_model(
        ...
        Args:
            ...
            device: Device to load the ASR model on. If None, auto-selects GPU if available.
        """
        from transformers import pipeline as hf_pipeline

        # Default to GPU if available and device is not specified
        if device is None:
            asr_device = "cuda" if torch.cuda.is_available() else ("xpu" if hasattr(torch, "xpu") and torch.xpu.is_available() else ("mps" if torch.backends.mps.is_available() else "cpu"))
        else:
            asr_device = device
            
        logger.info("Loading ASR model %s on %s ...", model_name, asr_device)
        asr_dtype = (
            torch.float16
            if str(asr_device).startswith(("cuda", "xpu"))
            else torch.float32
        )
        ...
        logger.info("ASR model loaded on %s.", asr_device)
    ...
    def _prepare_embed_inputs(
        ...
        if self.token_id_mapping is not None:
            # Map input text ids to pruned vocabulary ids
            mapping = self.token_id_mapping.to(text_ids.device)
            # Ensure text_ids are within bounds for mapping
            clamped_text_ids = torch.clamp(text_ids, 0, len(mapping) - 1)
            mapped_text_ids = mapping[clamped_text_ids]
            text_embeds = self.get_input_embeddings()(mapped_text_ids)
        else:
